...
function showBrowserMessage(messageText) {
  ...
  const first = () => {
    showElement(browserMessage);
  };
  const second = () => {
    setTimeout(function () {
      hideElement(browserMessage);
    }, 1000);
  };
  first();
  second();
...
const trySave = (size, layout, fonts, svg, x, y, width, height, name) => {
  const message = {
    loading:
      "Загрузка скоро начнётся....<img src='./img/spinner.svg' alt='spinner'><br> Попытка № ",
...
const toZero = (e, value1, fixed) => {
  // К value1 при blur, fixed(2) введённое значение
  if (!e.target.value) {
...
const delVaule = (e) => {
  // Удаление значения при focus
  e.target.value = "";
};
const enter = (e) => {
  // blur при нажатии на Enter
  if (e.key == "Enter") {
    e.target.blur();
  }
};
const btnStatus = () => {
  //Активность кнопки при отсутствии ночных или дневных смен
...
tableWrapper.addEventListener("click", (e) => {
  ...
  if (
    (e.target && e.target.matches(".input-number__plus")) ||
    e.target.matches(".input-number__minus")
  ) {
    if (
      (nextInput && nextInput.classList.contains("dayhours")) ||
      (previousInput && previousInput.classList.contains("dayhours"))
    ) {
    ...
    if (
      (nextInput && nextInput.classList.contains("nighthours")) ||
      (previousInput && previousInput.classList.contains("nighthours"))
    ) {
    ...
    if (
      (nextInput && nextInput.matches("#mh")) ||
      (previousInput && previousInput.matches("#mh"))
    ) {
      btnStatus();
      mdgVal();
    ...
    if (
      (nextInput && nextInput.matches("#nh")) ||
      (previousInput && previousInput.matches("#nh"))
    ) {
      btnStatus();
      ndgVal();
    ...
    if (
      (nextInput && nextInput.matches("#mhg")) ||
      (previousInput && previousInput.matches("#mhg"))
    ) {
      if (+mhg.value <= +mh.value) {
        mhg.value = mh.value;
        mdg.value = md.value;
    ...
    if (
      (nextInput && nextInput.matches("#nhg")) ||
      (previousInput && previousInput.matches("#nhg"))
    ) {
      if (+nhg.value <= +nh.value) {
        nhg.value = nh.value;
        ndg.value = nd.value;
...
function setSheetSize() {
  ...
  let tableAndBtnsHight =
    +window.getComputedStyle(tableAndBtns).height.match(/\d/gim).join("") - 40;
  tableAndBtnsHight > 1000
    ? (tableAndBtnsHight = tableAndBtnsHight / 1000)
    : null;
  sheet.style = `width:${tableAndBtnsHight / 1.5}px;`;
  console.log(tableAndBtnsHight);
  createTable();
}
